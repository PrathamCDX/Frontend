...
export default function SearchCandidatesCard() {
  ...
  const { data: candidatesByName, isPending: isPendingByName } =
    useSearchCandidatesByName(jwtToken, debouncedValue, 1, 10);
  const {
    data: candidatesByEmail,
    isPending: isPendingByEmail,
    refetch,
  } = useSearchCandidatesByEmail(jwtToken, submitEmailSearch);
  ...
  return (
    <div className="space-y-4 relative py-5 flex flex-col items-center w-full h-full">
      ...
      {name.length == 0 && emailInput.length == 0 ? (
        <div>Search using email or name only</div>
        ...
        <div className="w-full flex-1 min-h-0 max-h-full overflow-y-scroll">
          {name.length > 0 &&
          candidatesByName &&
          candidatesByName?.rows.length > 0 ? (
            <div>
            ...
            <div>
              ...
              >
                ...
                <div>
                  {"Graduation Year: "}
                  {candidatesByEmail.graduationYear ?? "Not available"}
                </div>
              </div>
            </div>
          ) : (
