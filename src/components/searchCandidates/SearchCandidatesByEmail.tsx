...
export default function SearchCandidatesByEmail() {
  ...
  const { data, isFetching } = useSearchCandidatesByEmail(jwtToken, emailParam);
  ...
  return (
    <div className="flex flex-col h-full w-full gap-4 min-h-0 relative">
      ...
      <div className="text-xl my-3 text-center">Search Candidates By Email</div>
        ...
        {data ? (
          <div className="p-4">
            <div className="border rounded-lg p-4">
              ...
              <div>
                <b>Graduation Year:</b> {data.graduationYear ?? "Not available"}
              </div>
            </div>
          </div>
        ) : (
