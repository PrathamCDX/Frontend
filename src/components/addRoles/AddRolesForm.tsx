...
export default function AddRoles() {
  ...
  const {
    ...
    refetch,
    isLoading,
  } = useGetRoles(authJwtToken, debouncedInput);
  ...
  const isDisabled =
    isLoading ||
    (roleNames ? roleNames?.length > 0 : false) ||
    debouncedInput.trim() === "";

  return (
    <div className="px-8 py-3 relative">
      <button
        className="absolute top-4 right-1 hover:cursor-pointer"
        onClick={() => {
          dispatch(setShowAddRolesForm(false));
        }}
      >
        <X />
      </button>
      <div className="text-center text-2xl font-semibold">Add Roles</div>
      <div className="my-3 ">
        <div className="flex rounded-t-2xl w-full border overflow-hidden ">
          ...
          <button
            disabled={isDisabled}
            className={`bg-blue-500 px-6 ${isDisabled ? " hover:cursor-not-allowed " : " hover:cursor-pointer "} `}
            onClick={() => {
              mutate({ authJwtToken: authJwtToken, roleName: debouncedInput });
            }}
          >
            Create
          </button>
